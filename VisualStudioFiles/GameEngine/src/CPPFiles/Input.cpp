...
GLFWwindow* Input::gameWindow;
Input Input::instance;

bool Input::cursorVisible = true;

std::unordered_map<KeyCode, bool> Input::keyDown;
std::unordered_map<MouseButtonCode, bool> Input::mouseButtonDown;
...
bool Input::MouseButtonUp(MouseButtonCode button) {
	return (glfwGetMouseButton(gameWindow, (int)button) == GLFW_RELEASE);
}

bool Input::MouseButtonDown(MouseButtonCode button) {
	return mouseButtonDown[button];
}

bool Input::MouseButtonHeld(MouseButtonCode button) {
	return (glfwGetMouseButton(gameWindow, (int)button) == GLFW_PRESS);
}


void Input::SetMouseVals() {

	double posX;
	double posY;
	glfwGetCursorPos(gameWindow, &posX, &posY);

	if (firstMouse) {
		glfwGetCursorPos(gameWindow, &lastMousePosX, &lastMousePosY);
		firstMouse = false;
	}
	else {
	...
	mouseDeltaX = posX - lastMousePosX;
	mouseDeltaY = posY - lastMousePosY;
}
